...
def migrate_objectids(db):
    ...
    from migrate_objectids import migrate_array_field as _migrate_array_field
    from migrate_objectids import migrate_field as _migrate_field

    def migrate_field(collection_name: str, field: str):
        return _migrate_field(db, collection_name, field)

    def migrate_array_field(collection_name: str, field: str):
        return _migrate_array_field(db, collection_name, field)
    ...
    # reports
    total += migrate_field("reports", "exerciseId")
    total += migrate_field("reports", "userId")
    # users
