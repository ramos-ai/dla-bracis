...
def save_submission(data: dict) -> dict:
    ...
    if not doc:
        doc = {
            "userId": user_oid or user_id,
            "exerciseId": exercise_oid or exercise_id,
            "labelledAnswers": [],
            "unlabelledAnswers": [],
            "submittedAt": utc_now(),
    ...
    try:
        submissions_collection.update_one(
            filter_criteria, {"$set": update_data}, upsert=True
        )
        ...
        if data.get("labelledAnswers") and data.get("dataset_id"):
            _refresh_classification_reliability(user_id, data.get("dataset_id"))
...
def _refresh_classification_reliability(user_id: str, dataset_id: str) -> None:
    """Best-effort recompute of Cohen κ weight after supervised classification answers."""
    try:
        from application.students.update_classification_reliability import (
            update_classification_reliability,
        )

        update_classification_reliability(str(user_id), dataset_id=str(dataset_id))
    except Exception:
        logger.warning(
            "Failed to refresh classification reliability user_id=%s dataset_id=%s",
            user_id,
            dataset_id,
            exc_info=True,
        )
