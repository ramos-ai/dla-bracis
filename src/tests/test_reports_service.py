...
class TestBuildReportPayload:
    def test_converts_valid_ids_to_objectid(self):
        from application.reports.reports_service import _build_report_payload
        ...
        assert payload["exerciseId"] == ObjectId(exercise_id)
        assert payload["userId"] == ObjectId(user_id)

    def test_keeps_invalid_ids_as_string(self):
        from application.reports.reports_service import _build_report_payload

        payload = _build_report_payload(
            {
                "exerciseId": "not-an-object-id",
                "userId": "also-not-one",
                "reportType": "bug",
                "description": "desc",
            }
        ...
        assert payload["exerciseId"] == "not-an-object-id"
        assert payload["userId"] == "also-not-one"
