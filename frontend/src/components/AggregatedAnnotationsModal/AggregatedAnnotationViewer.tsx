...
const AggregatedAnnotationViewer: React.FC<AggregatedAnnotationViewerProps> = ({
  ...
  const drawSegmentationAnnotations = (
    ...
    annotations.forEach((ann) => {
      ...
      // Decided once per polygon (not per point): a single point sitting at/near
      // the origin in pixel space (e.g. a shape touching the image's top-left
      // corner) would otherwise be misread as normalized on its own, distorting
      // the rest of an already-consistent polygon.
      const isNormalized = polygon.every((v) => v <= 1);
